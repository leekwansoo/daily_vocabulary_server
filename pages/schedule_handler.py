...
selected = st.sidebar.radio("Select an option", ["Subscriber_List", "Select Vocabulary", "View_selected_words_with_audio", "Mail Words"])
if selected == "Subscriber_List":
    ...
    current_level =st.radio("Select Library", ["level1", "level2", "level3", "mailed"], horizontal=True)
...
elif selected == "View_selected_words_with_audio": 
    st.write("This section would display the selected words list.")
    button = st.button("View Today's Words")
    if button:
        result = asyncio.run(view_trigger())
        print(result)
        
        if result:
            status = result.get('status', '')
            if status == 'no_selected_words':
                st.info("No words scheduled to be viewed today.")
            elif status == 'viewed_and_marked_sent':
                selected_words = result.get('selected_words', [])
                st.success(f"Email sent successfully with {len(selected_words)} words!")
