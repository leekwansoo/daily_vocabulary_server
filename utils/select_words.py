...
def load_seq_no():
    ...
    try:
        ...
        seq_file = os.path.join(project_root, 'seq_state.json')
...
def select_words_from_vocabulary(number_of_words, selection_method, current_level, seq_no=0):
    ...
    level_files = {
        1: os.path.join(project_root, "level1.json"),
        2: os.path.join(project_root, "level2.json"), 
        3: os.path.join(project_root, "level3.json")
    }
    
    filename = level_files.get(current_level)
    print(f"Loading vocabulary from file: {filename}")
    all_words = load_vocabulary_from_file(filename)
    print(f"Total words available for selection: {len(all_words)}, in level {current_level}")
...
if hasattr(st.session_state, 'selected_words') and st.session_state.selected_words:
    if st.button("Save Selected Words to Selected List"):
        print("Saving selected words to selected.json...")
        selected_words = st.session_state.selected_words
        result = save_selected_words(selected_words)
        print(result)
        if result:
            st.success(f"✅ Saved {len(selected_words)} words to selected.json")
